...
Cqt::ResamplingFilterbank::ResamplingFilterbank(int stageNumber)
{
	mDownsamplingFilters.resize(stageNumber - 1);
	mUpsamplingFilters.resize(stageNumber - 1);
	mStageInputBuffers.resize(stageNumber);
	mStageOutputBuffers.resize(stageNumber);
	mStageSamplerates.resize(stageNumber);
	mDownsamplingBlockSizes.resize(stageNumber - 1);
	mUpsamplingBlockSizes.resize(stageNumber - 1);
}

void Cqt::ResamplingFilterbank::inputBlock(float* const data, const int blockSize)
{
	...
	if (mInputDataCounter >= mExpectedBlockSize)
	{
		...
		float* dataIn = mInputResamplingHandler.processBlockDown(mInputData.data());
...
float* Cqt::ResamplingFilterbank::outputBlock(const int blockSize)
{
	...
	if (mOutputDataCounter >= mExpectedBlockSize)
	{
		...
		//Filterbank
		int stageIdx = getStageNumber() - 1;
		int filterIdx = getStageNumber() - 2;
		// handle sample based stages
		for (int s = 0; s < mSampleInputSize; s++)
		{
			stageIdx = getStageNumber() - 1;
			filterIdx = getStageNumber() - 2;
			// lowest Stage
		...
		float* inBlock = mUpsamplingSampleOutputBuffer.data();
		if (mSampleFilterNumber <= 0)
		{
		...
		// input Resampler to FsOrigin
		float* dataOut = mInputResamplingHandler.processBlockUp(inBlock);
		// store output
...
float Cqt::ResamplingFilterbank::getOriginSamplerate()
{
	return mOriginSamplerate;
}

int Cqt::ResamplingFilterbank::getOriginBlockSize()
{
	return mOriginBlockSize;
}

int Cqt::ResamplingFilterbank::getStageNumber() const
{
	return mStageInputBuffers.size();
}

Cqt::BufferPtr Cqt::ResamplingFilterbank::getStageInputBuffer(const int stage)
{
	return &mStageInputBuffers[stage];
}

Cqt::BufferPtr Cqt::ResamplingFilterbank::getStageOutputBuffer(const int stage)
{
	return &mStageOutputBuffers[stage];
}

int Cqt::ResamplingFilterbank::getOriginDownsampling()
{
	return mOriginDownsampling;
}

void Cqt::ResamplingFilterbank::init(const float samplerate, const int blockSize, const int bufferSize)
{
	...
	while ((originBlockSize > 1) && (mBlockFilterNumber < (getStageNumber() - 1)) && ((originBlockSize % 2) == 0))
	{
	...
	// init Filters
	for (int stage = 0; stage < (getStageNumber() - 1); stage++)
	{
	...
	if ((getStageNumber() - 1) > mBlockFilterNumber)
	{
		mSampleInputSize = originBlockSize;
		mUpsamplingSampleOutputBuffer.resize(originBlockSize, 0.);
		mSampleFilterNumber = (getStageNumber() - 1) - mBlockFilterNumber;
	...
	// init Buffers
	for (int stage = 0; stage < getStageNumber(); stage++)
	{
