...
void QWindow3DEditor::paintEvent(QPaintEvent* event)
{
	...
	//painter.drawPolygon(QPolygonF({ mapFromGlobal(mVertex[0]->getGlobalPos()) ,mapFromGlobal(mVertex[1]->getGlobalPos()) ,mapFromGlobal(mVertex[2]->getGlobalPos()) ,mapFromGlobal(mVertex[3]->getGlobalPos()) }));
}
